#!/usr/bin/env node
/**
 * Adds farmer background columns to public.profiles in Supabase.
 *
 * Option A — database URI (recommended):
 *   SUPABASE_DB_URL=postgresql://postgres.[ref]:[password]@... npm run db:migrate-farmer-background
 *
 * Option B — Supabase Management API:
 *   SUPABASE_ACCESS_TOKEN=sbp_... npm run db:migrate-farmer-background
 */
...
const migrationSql = readFileSync(join(root, 'supabase/add-farmer-background-columns.sql'), 'utf8');
...
async function verifyColumns() {
  ...
  const response = await fetch(
    `${url}/rest/v1/profiles?select=years_farming,farm_role,primary_goals&limit=1`,
    {
      headers: {
        apikey: key,
  ...
  if (response.ok) {
    console.log('Verified: farmer background columns are visible to the API.');
    return;
  }
...
Add ONE of these to your .env file, then run:
  npm run db:migrate-farmer-background
